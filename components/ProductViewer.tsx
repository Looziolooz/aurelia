...
function Lights() {
  ...
  return (
    <>
       ...
       *  bronze, not a glowing orange; gives the bezel/collar a touch of
       *  local warmth so it doesn't read as grey plastic.
       *  PASS 4c (root cause): sits front-right, right where the gauge
       *  cluster is — at 1.6 it was the orange-white CORE of the gauge
       *  blowout (3 material passes couldn't beat a direct light this
       *  strong). 1.6→0.7: still warms the bezel/collar, no longer blows
       *  the convex gauge at glancing angles. DO NOT raise back. */}
      <pointLight
        position={[2.4, 2.5, 3]}
        intensity={0.7 * k}
        distance={8}
       ...
       *  the floor below. Warm ivory #f4e4ca (atelier-tungsten, within
       *  the quiet-museum / copper brand, not amber/sodium).
       *  PASS 4c (root cause, ONE move fixes TWO reported issues): at 12
       *  (×1.4 on BLACK ≈ 17) it both (a) blew the gauge dome white and
       *  (b) made the over-bright floor pool under the cups ("ripiano
       *  troppo luminoso"). 12→6.5: still lays the top-edge band and a
       *  calm floor presence, no longer clips. Aligned with the LOCKED
       *  brief ("no shouting, lusso quieto"). DO NOT raise back: KEY
       *  (0.72) + FILL (0.45) + hemi (0.5) carry the form. */}
      <spotLight
        position={[0, 8, 1]}
        angle={0.6}
        penumbra={0.95}
